...
export interface MermbershipProps extends Modify<MembershipParams, {
}> {}
...
export class Membership {
  ...
  constructor(params?: MembershipParams) {
    if (params) this.initWithParams(params); 
    if (!params) this.initEmpty(); 
  }
  initWithParams(params: MembershipParams) {
  ...
  get costo() {return Intl.NumberFormat('en-MX',{minimumFractionDigits: 2}).format(this._props.costo)}
  get duracion() {return this._props.duracion}
  get hide() {return this._props.hide}
  get limitada() {return this._props.limitada}
  get nombre() { 
    if(this._props.nombre.length <= 40){
      return this._props.nombre
    }
    else{
      let cad = this._props.nombre.substr(0,37)+'...';
      return cad;
    }
  }
  ...
  set costo(cost) { this._props.costo = parseInt(cost) }
